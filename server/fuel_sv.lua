...
if Config.RenewedPhonePayment then
	RegisterNetEvent('cdn-fuel:server:phone:givebackmoney', function(amount)
		...
		player.Functions.AddMoney("bank", math.ceil(amount), Lang:t("phone.phone_refund_payment_label"))
	end)
end
...
RegisterNetEvent("cdn-fuel:server:OpenMenu", function(amount, inGasStation, hasWeapon, purchasetype, FuelPrice)
	...
	if amount < 1 then TriggerClientEvent('QBCore:Notify', src, Lang:t("fuel.more_than_zero"), 'error') return end
	if inGasStation == true and not hasWeapon then
		if Config.RenewedPhonePayment and purchasetype == "bank" then
			...
			TriggerClientEvent('qb-menu:client:openMenu', src, {
				{
					header = Lang:t("menu.menu_refuel_header"),
					isMenuHeader = true,
				...
				},
				{
					...
					isMenuHeader = true,
					txt = Lang:t("menu.menu_purchase_station_header_1")..total..Lang:t("menu.menu_purchase_station_header_2") ,
				},
				{
					header = Lang:t("menu.menu_purchase_station_confirm_header"),
					icon = "fas fa-check-circle",
					txt = Lang:t("menu.menu_refuel_accept"),
					params = {
...
RegisterNetEvent("cdn-fuel:server:PayForFuel", function(amount, purchasetype, FuelPrice, electric)
	...
	local payString = Lang:t("menu.menu_pay_label_1") ..FuelPrice..Lang:t("menu.menu_pay_label_2")
	if electric then payString = Lang:t("menu.menu_electric_payment_label_1") ..FuelPrice..Lang:t("menu.menu_electric_payment_label_2") end
...
RegisterNetEvent("cdn-fuel:server:purchase:jerrycan", function(purchasetype)
	...
	if Player.Functions.AddItem("jerrycan", 1, false, info) then -- Dont remove money if AddItem() not possible!
		...
		Player.Functions.RemoveMoney(moneyremovetype, total, Lang:t("jerrycan.jerry_can_payment_label"))
	end
end)
