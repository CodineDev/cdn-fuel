if Config.PlayerOwnedGasStationsEnabled then -- This is so Player Owned Gas Stations are a Config Option, instead of forced. Set this option in shared/config.lua!
        ...
        for i = 1, #Config.GasStations do
            ...
            exports['qb-target']:AddTargetEntity(ped, {
                options = {
                    {
                        type = "client",
                        label = Lang:t("target.station_talk_to_ped"),
    ...
    RegisterNetEvent('cdn-fuel:stations:client:purchaselocation', function(data)
        ...
        elseif IsOwned then 
            QBCore.Functions.Notify(Lang:t("stations.station_already_owned"), 'error', 7500)
        end
    end)
    ...
    RegisterNetEvent('cdn-fuel:stations:client:sellstation', function(data)
        ...
        QBCore.Functions.TriggerCallback('cdn-fuel:server:isowner', function(result)
            if result then
                ...
                QBCore.Functions.Notify(Lang:t("stations.station_not_owner"), 'error', 7500)
        ...
        if CanSell then
            ...
            QBCore.Functions.Notify(Lang:t("stations.station_cannot_sell"), 'error', 7500)
        end
    end)
    ...
    RegisterNetEvent('cdn-fuel:stations:client:purchasereserves:final', function(location, price, amount) -- Menu, seens after selecting the "purchase reserves" option.
        ...
        if CanOpen then
            ...
            exports['qb-menu']:openMenu({
                {
                    header = Lang:t("menu.menu_station_reserves_header")..Config.GasStations[location].label,
                    isMenuHeader = true,
                    icon = "fas fa-gas-pump",
                },
                {
                    header = Lang:t("menu.menu_station_reserves_purchase_header")..price,
                    txt = Lang:t("menu.menu_station_reserves_purchase_footer")..price.."!", 
                    icon = "fas fa-usd",
                    params = {
                ...
                },
                {
                    ...
                    txt = Lang:t("menu.menu_station_reserves_cancel_footer"), 
    ...
    RegisterNetEvent('cdn-fuel:stations:client:purchasereserves', function(data)
        ...
        if CanOpen then
            ...
            local reserves = exports['qb-input']:ShowInput({
                header = Lang:t("input.input_purchase_reserves_header_1") .. Lang:t("input.input_purchase_reserves_header_2") .. Currentreserveamount .. Lang:t("input.input_purchase_reserves_header_3") ..
                ...
                submitText = Lang:t("input.input_purchase_reserves_submit_text"),
                inputs = { {
                    type = 'number',
                    isRequired = true,
                    name = 'amount',
                    text = Lang:t("input.input_purchase_reserves_text")
                }}
            })
            if reserves then
                ...
                if not reserves.amount then QBCore.Functions.Notify(Lang:t("stations.station_amount_invalid"), 'error', 7500) return end
                Reservebuyamount = tonumber(reserves.amount)
                if Reservebuyamount < 1 then QBCore.Functions.Notify(Lang:t("stations.station_more_than_one"), 'error', 7500) return end
                if (Reservebuyamount + Currentreserveamount) > Config.MaxFuelReserves then
                    QBCore.Functions.Notify(Lang:t("stations.station_reserve_cannot_fit"), "error")
                else
                    if GlobalTax(Reservebuyamount * Config.FuelReservesPrice) + (Reservebuyamount * Config.FuelReservesPrice) <= bankmoney then
                        ...
                        QBCore.Functions.Notify(Lang:t("fuel.not_enough_money_in_bank"), 'error', 7500)
                    end
                end    
            end
        end
    ...
    RegisterNetEvent('cdn-fuel:stations:client:changefuelprice', function(data) 
        ...
        if CanOpen then
            ...
            local fuelprice = exports['qb-input']:ShowInput({
                header = Lang:t("input.input_alter_fuel_price_header_1")..StationFuelPrice..Lang:t("input.input_alter_fuel_price_header_2"),
                submitText = Lang:t("input.input_alter_fuel_price_submit_text"),
                inputs = { {
                    type = 'number',
                    isRequired = true,
                    name = 'price',
                    text = Lang:t("input.input_alter_fuel_price_submit_text")
                }}
            })
            if fuelprice then
                if Config.FuelDebug then print("Attempting to change fuel price!") end
                Wait(100)
                if not fuelprice.price then QBCore.Functions.Notify(Lang:t("stations.station_amount_invalid"), 'error', 7500) return end
                NewFuelPrice = tonumber(fuelprice.price)
                if NewFuelPrice < Config.MinimumFuelPrice then QBCore.Functions.Notify(Lang:t("stations.station_price_too_low"), 'error', 7500) return end
                if NewFuelPrice > Config.MaxFuelPrice then
                    QBCore.Functions.Notify(Lang:t("stations.station_price_too_high"), "error")
                else
    ...
    RegisterNetEvent('cdn-fuel:stations:client:sellstation:menu', function(data) -- Menu, seen after selecting the Sell this Location option.
        ...
        if CanOpen then
            ...
            exports['qb-menu']:openMenu({
                {
                    header = Lang:t("menu.menu_sell_station_header")..Config.GasStations[location].label,
                    isMenuHeader = true,
                    icon = "fas fa-gas-pump",
                },
                {
                    header = Lang:t("menu.menu_sell_station_header_accept"),
                    txt = Lang:t("menu.menu_sell_station_footer_accept")..SalePrice..".", 
                    icon = "fas fa-usd",
                    params = {
                ...
                {
                    header = Lang:t("menu.menu_header_close"),
                    txt = Lang:t("menu.menu_sell_station_footer_close"), 
                    icon = "fas fa-times-circle",
                    params = {
                        event = "qb-menu:closeMenu",
                    }
    ...
    RegisterNetEvent('cdn-fuel:stations:client:changestationname', function() -- Menu for changing the label of the owned station.
        ...
        if CanOpen then
            ...
            local NewName = exports['qb-input']:ShowInput({
                header = Lang:t("input.input_change_name_header_1")..Config.GasStations[CurrentLocation].label..Lang:t("input.input_change_name_header_2"),
                submitText = Lang:t("input.input_change_name_submit_text"),
                inputs = { {
                    type = 'text',
                    isRequired = true,
                    name = 'newname',
                    text = Lang:t("input.input_change_name_text")
                }}
            })
            if NewName then
                if Config.FuelDebug then print("Attempting to alter stations name!") end
                if not NewName.newname then QBCore.Functions.Notify(Lang:t("stations.station_name_invalid"), 'error', 7500) return end
                NewName = NewName.newname
                if type(NewName) ~= "string" then QBCore.Functions.Notify(Lang:t("stations.station_name_invalid"), 'error') return end
                if Config.ProfanityList[NewName] then QBCore.Functions.Notify(Lang:t("stations.station_name_invalid"), 'error', 7500) 
                    -- You can add logs for people that put prohibited words into the name changer if wanted, and here is where you would do it.
                    return 
                end
                if string.len(NewName) > Config.NameChangeMaxChar then QBCore.Functions.Notify(Lang:t("stations.station_name_too_long"), 'error') return end
                if string.len(NewName) < Config.NameChangeMinChar then QBCore.Functions.Notify(Lang:t("stations.station_name_too_short"), 'error') return end
                Wait(100)
    ...
    RegisterNetEvent('cdn-fuel:stations:client:managemenu', function(location) -- Menu, seen after selecting the Manage this Location Option.
        ...
        if CanOpen then
            ...
            exports['qb-menu']:openMenu({
                {
                    header = Lang:t("menu.menu_manage_header")..Config.GasStations[location].label,
                    isMenuHeader = true,
                    icon = "fas fa-gas-pump",
                },
                {
                    header = Lang:t("menu.menu_manage_reserves_header"),
                    icon = "fas fa-info-circle",
                    isMenuHeader = true,
                    txt = ReserveLevels..Lang:t("menu.menu_manage_reserves_footer_1")..Config.MaxFuelReserves..Lang:t("menu.menu_manage_reserves_footer_2"),
                },
                {
                    header = Lang:t("menu.menu_manage_purchase_reserves_header"),
                    icon = "fas fa-usd",
                    txt = Lang:t("menu.menu_manage_purchase_reserves_footer")..Config.FuelReservesPrice..Lang:t("menu.menu_manage_purchase_reserves_footer_2") ,
                    params = {
                ...
                {
                    header = Lang:t("menu.menu_alter_fuel_price_header"),
                    icon = "fas fa-usd",
                    txt = Lang:t("input.input_change_name")..StationFuelPrice..Lang:t("input.input_alter_fuel_price_header_2") ,
                    params = {
                ...
                {
                    header = Lang:t("menu.menu_manage_company_funds_header"),
                    icon = "fas fa-usd",
                    txt = Lang:t("menu.menu_manage_company_funds_footer"),
                    params = {
                        event = "cdn-fuel:stations:client:managefunds",
                    },
                },
                {
                    header = Lang:t("menu.menu_manage_change_name_header"),
                    icon = "fas fa-pen",
                    txt = Lang:t("menu.menu_manage_change_name_footer"),
                ...
                {
                    header = Lang:t("menu.menu_sell_station_header_accept"),
                    txt = Lang:t("menu.menu_manage_sell_station_footer")..math.percent(Config.GasStationSellPercentage, GasStationCost), 
                    icon = "fas fa-usd",
                    params = {
                ...
                {
                    header = Lang:t("menu.menu_header_close"),
                    txt = Lang:t("menu.menu_manage_close"), 
                    icon = "fas fa-times-circle",
                    params = {
                        event = "qb-menu:closeMenu",
                    }
    ...
    RegisterNetEvent('cdn-fuel:stations:client:managefunds', function(location) -- Menu, seen after selecting the Manage this Location Option.
        ...
        if CanOpen then
            exports['qb-menu']:openMenu({
                {
                    header = Lang:t("menu.menu_manage_company_funds_header_2")..Config.GasStations[CurrentLocation].label,
                    isMenuHeader = true,
                    icon = "fas fa-gas-pump",
                },
                {
                    header = Lang:t("menu.menu_manage_company_funds_withdraw_header"),
                    icon = "fas fa-arrow-left",
                    txt = Lang:t("menu.menu_manage_company_funds_withdraw_footer"),
                    params = {
                ...
                {
                    header = Lang:t("menu.menu_manage_company_funds_deposit_header"),
                    icon = "fas fa-arrow-right",
                    txt = Lang:t("menu.menu_manage_company_funds_deposit_footer"),
                    params = {
                ...
                {
                    header = Lang:t("menu.menu_manage_company_funds_return_header"),
                    txt = Lang:t("menu.menu_manage_company_funds_return_footer"), 
    ...
    RegisterNetEvent('cdn-fuel:stations:client:WithdrawFunds', function(data)
        ...
        if CanOpen then
            ...
            local Withdraw = exports['qb-input']:ShowInput({
                header = Lang:t("input.input_withdraw_funds_header") ..StationBalance,
                submitText = Lang:t("input.input_withdraw_submit_text"),
                inputs = { {
                    type = 'number',
                    isRequired = true,
                    name = 'amount',
                    text = Lang:t("input.input_withdraw_text")
                }}
            })
            if Withdraw then
                ...
                if not Withdraw.amount then QBCore.Functions.Notify(Lang:t("stations.station_amount_invalid"), 'error', 7500) return end
                if amount < 1 then QBCore.Functions.Notify(Lang:t("stations.station_withdraw_too_little"), 'error', 7500) return end
                if amount > StationBalance then QBCore.Functions.Notify(Lang:t("stations.station_withdraw_too_much"), 'error', 7500) return end
                WithdrawAmount = tonumber(amount)
                if (StationBalance - WithdrawAmount) < 0 then
                    QBCore.Functions.Notify(Lang:t("stations.station_withdraw_too_much"), 'error', 7500)
                else
    ...
    RegisterNetEvent('cdn-fuel:stations:client:DepositFunds', function(data)
        ...
        if CanOpen then
            ...
            local Deposit = exports['qb-input']:ShowInput({
                header = Lang:t("input.input_deposit_funds_header") ..StationBalance,
                submitText = Lang:t("input.input_deposit_submit_text"),
                inputs = { {
                    type = 'number',
                    isRequired = true,
                    name = 'amount',
                    text = Lang:t("input.input_deposit_text")
                }}
            })
            if Deposit then
                ...
                if not Deposit.amount then QBCore.Functions.Notify(Lang:t("stations.station_amount_invalid"), 'error', 7500) return end
                if amount < 1 then QBCore.Functions.Notify(Lang:t("stations.station_deposit_too_little"), 'error', 7500) return end
                DepositAmount = tonumber(amount)
                if (DepositAmount) > bankmoney then
                    QBCore.Functions.Notify(Lang:t("stations.station_deposity_too_much"), "error")
                else
    ...
    RegisterNetEvent('cdn-fuel:stations:client:purchasemenu', function(location) -- Menu, seen after selecting the purchase this location option.
        ...
        if bankmoney < costofstation then
            QBCore.Functions.Notify(Lang:t("fuel.not_enough_money_in_bank").." $"..costofstation, 'error', 7500) return
        end
        ...
        exports['qb-menu']:openMenu({
            ...
            },
            {
                ...
                icon = "fas fa-info-circle",
                isMenuHeader = true,
                txt = Lang:t("menu.menu_purchase_station_header_1")..costofstation..Lang:t("menu.menu_purchase_station_header_2"),
            },
            {
                header = Lang:t("menu.menu_purchase_station_confirm_header"),
                icon = "fas fa-check-circle",
                txt = Lang:t("menu.menu_purchase_station_confirm_footer")..costofstation..'!' ,
                params = {
            ...
            {
                header = Lang:t("menu.menu_header_close"),
                txt = Lang:t("menu.menu_purchase_station_cancel_footer"), 
                icon = "fas fa-times-circle",
                params = {
                    event = "qb-menu:closeMenu",
                }
    ...
    RegisterNetEvent('cdn-fuel:stations:openmenu', function() -- Menu #1, the first menu you see.
        ...
        exports['qb-menu']:openMenu({
            ...
            {
                header = Lang:t("menu.menu_ped_manage_location_header"),
                txt = Lang:t("menu.menu_ped_manage_location_footer"),
                icon = "fas fa-usd",
                params = {
                    event = "cdn-fuel:stations:client:managemenu",
            ...
            {
                header = Lang:t("menu.menu_ped_purchase_location_header"),
                txt = Lang:t("menu.menu_ped_purchase_location_footer"),
                icon = "fas fa-usd",
                params = {
            ...
            {
                header = Lang:t("menu.menu_ped_emergency_shutoff_header"),
                txt = Lang:t("menu.menu_ped_emergency_shutoff_footer")..PumpState,
                icon = "fas fa-gas-pump",
                params = {
            ...
            {
                header = Lang:t("menu.menu_ped_close_header"),
                txt = Lang:t("menu.menu_ped_close_footer"),
                icon = "fas fa-times-circle",
                params = {
                    event = "qb-menu:closeMenu",
                }
